...
export class JobService {
    ...
    getJobsByCompany(companyId: number): Observable<Job[]> {
        // Embed applications to get real counts (used by dashboard stats)
        return this.http.get<Job[]>(`${this.apiUrl}?companyId=${companyId}&_embed=applications`);
    }

    // Simple fetch without embed - used by PostedJobs child component
    getJobsByCompanySimple(companyId: number): Observable<Job[]> {
